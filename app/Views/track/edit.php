...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Upravit skladbu</li>
    </ol>
</nav>

<h1 class="h3 mb-4">Upravit skladbu</h1>

<div class="card shadow-sm" style="max-width: 600px;">
    <div class="card-body">
        <form action="<?= base_url('album/' . $album->id . '/track/' . $track->id . '/update') ?>" method="post">
            <?= csrf_field() ?>

            <div class="mb-3">
                <label for="title" class="form-label">Název skladby <span class="text-danger">*</span></label>
                <input type="text" class="form-control" id="title" name="title"
                       value="<?= esc(old('title', $track->title)) ?>" required>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control" id="track_number" name="track_number"
                           value="<?= esc(old('track_number', $track->track_number ?? '')) ?>" min="1">
                </div>
                <div class="col-md-6">
                    <label for="duration" class="form-label">Délka (v sekundách)</label>
                    <input type="number" class="form-control" id="duration" name="duration"
                           value="<?= esc(old('duration', $track->duration ?? '')) ?>" min="0">
                </div>
            </div>
